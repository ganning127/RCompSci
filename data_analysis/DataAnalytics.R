...
setwd("~/programming/programming_ncssm/rcompsci/data_analysis")  #  all files must be in the 
#  designated folder
#
# read in the myfunctions.R file
source("~/programming/programming_ncssm/rcompsci/data_analysis/myfunctions.R")
#
...
# Column 2 ("Imports") is character data.  To use the random.impute
...
data <- random.impute.data.frame(data, c(2))
...
plotluck(data, .~1)
data[cols] <- lapply(data[cols], rz.transform)
plotluck(data, .~1)
#
...
glimpse(data)
data <- rename(data, "MeatRatio" = "meat_ratio")
data <- rename(data, "DairyRatio" = "dairy_ratio")
#

...
jittermap(cross) # create jittermap summary
cross <- est.rf(cross)
plotRF(cross) # plot the glowing line thing
plotMap(cross) # plot genetic map
plotMissing(cross) # shows that there are a lot of missing data in dipetrillo


# BLOOD PRESSURE
bp <- cross$pheno$Mean.BP # grab mean BP
hist(bp) # if this isn't normally distributed, using hist(log(bp)) can help
qqnorm(bp) # scatterplot
qqline(bp, col='red')  # draw best fit line
cross <- calc.genoprob(cross, step=2.0, off.end=0, error.prob=1.0e-4, map.function="haldane",stepwidth = 'fixed')
cross <- sim.geno(cross, step=2.0, off.end=0, error.prob=1.0e-4, map.function="haldane",stepwidth = 'fixed') # create mainscan preparation
cross.scanBP <- scanone(cross, pheno.col=3, model="normal", method="em")
cross.scanBP.perm <- scanone(cross, pheno.col=3, model="normal", method="em", n.perm=1000) # the higher the better (typically 1000 or more)
plot(cross.scanBP, main="Mainscan plot of Mean BP") 
thresh <- summary(cross.scanBP.perm, alpha=c(0.10, 0.05, 0.01)) # 90, 95, 99% confidence intervals
abline(h=thresh[1],col='blue') # 90%
abline(h=thresh[2],col='purple') # 95%
abline(h=thresh[3],col='green') # 99%
...
hist(hr) # create histogram of heart rate
cross <- calc.genoprob(cross, step=2.0, off.end=0, error.prob=1.0e-4, map.function="haldane",stepwidth = 'fixed')
...
cross.scanHR.perm <- scanone(cross, pheno.col=4, model="normal", method="em", n.perm=1000) # the higher the better (typically 1000 or more)
plot(cross.scanHR, main="Mainscan plot of Mean HR") 
thresh <- summary(cross.scanHR.perm, alpha=c(0.10, 0.05, 0.01)) # 90, 95, 99% confidence intervals
abline(h=thresh[1],col='blue') # draw confidence lines (same as above)
...
# HEART WEIGHT
wt <- cross$pheno$Heart.Wt # grab heart rate
hist(wt) # draw histogram
